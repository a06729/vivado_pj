...
module tb_spi_slave();
    ...
    wire [7:0] waddr1 = 8'h10; // 8'h10 = 8'b0001_0000
    wire [7:0] raddr1 = 8'h10;
    wire [7:0] wdata1 = 8'h45; // 8'h45 = 8'b0100_0101
    wire [7:0] waddr2 = 8'h11; // 8'h11 = 8'b0001_0001
    wire [7:0] raddr2 = 8'h11;
    wire [7:0] wdata2 = 8'h89; // 8'h89 = 8'b1000_1001
    ...
    always @(posedge clock or negedge reset) begin
        if(!reset) 
            ...
            cnt2 <= (cnt1 == 4'd15) ? cnt2 + 1'b1 : cnt2;
    end
    ...
    always @(posedge clock or negedge reset) begin
        if(!reset) 
            ss <= 1'b1; // ss : active high (idle 상태)
        else 
            ss <= ((cnt2==8'd10 ) && (cnt1==4'd0)) ? 1'b0 : // ss : active low
    ...
    always @(posedge clock or negedge reset) begin
        if(!reset) 
            ...
            sck <=  (cnt2>=8'd11 && cnt2<=8'd34)   ? ((cnt1==4'd0) ? 1'b1 : (cnt1==4'd8) ? 1'b0 : sck) :
                    (cnt2>=8'd61 && cnt2<=8'd84)   ? ((cnt1==4'd0) ? 1'b1 : (cnt1==4'd8) ? 1'b0 : sck) :
                    (cnt2>=8'd111 && cnt2<=8'd134) ? ((cnt1==4'd0) ? 1'b1 : (cnt1==4'd8) ? 1'b0 : sck) :
                    (cnt2>=8'd161 && cnt2<=8'd184) ? ((cnt1==4'd0) ? 1'b1 : (cnt1==4'd8) ? 1'b0 : sck) : 
                    sck ;
    end
    ...
    always @(posedge clock or negedge reset) begin
        if(!reset) 
            ...
            mosi <= ((cnt2==8'd10) && (cnt1==4'd0)) ? slave_idw[7] :
                    ...
                    ((cnt2>=8'd126 && cnt2<=8'd133) && (cnt1==4'd8)) ? 1'b0 : 
                    ...
                    ((cnt2>=8'd176 && cnt2<=8'd183) && (cnt1==4'd8)) ? 1'b0 : 
                    ((cnt2==8'd185) && (cnt1==4'd0)) ? 1'b1 : mosi ;
    end

    // MISO 읽기 블록 (★ 타이밍 1클럭씩 당김)
    wire miso;
    reg [7:0] rdata1;
