...
assign  s_idle = (m_state == M_IDLE) ? 1'b1 : 1'b0;
assign  s_receive = (m_state == M_RECEIVE)? 1'b1 : 1'b0;
assign s_done = (m_state == M_DONE ) ? 1'b1:1'b0;
...
assign cal_parity= rxd_data[0]^rxd_data[1]^rxd_data[2]^rxd_data[3]^rxd_data[4]^rxd_data[5]^rxd_data[6]^rxd_data[7];
...
always @(posedge mclk or negedge reset)begin
    ...
    else stop_bit2 <= ((parity_sel == 2'b00) & (cnt2==4'b10)&(cnt1 =={1'b0,baudrate[15:1]}))?rxd_3d:
                      ((parity_sel != 2'b00) & (cnt2==4'b11) & (cnt1 =={1'b0,baudrate[15:1]}))?rxd_3d:
                      stop_bit2;
end
...
always @(posedge mclk or negedge reset)begin
    if(~reset) frame_err <= 1'b0;
                      ...
                      ((cnt_done==3'd5) & (stop_sel==1'b1)) ? ~(stop_bit1&stop_bit2):
                      frame_err;
end
...
// [FIX1,2] output 포트는 내부 wire로 중계 후 assign
wire fifo_full;
wire fifo_empty;
wire fifo_valid;
wire [7:0] fifo_dout;
...
wire [7:0] fifo_din;
assign fifo_din = rxd_data;   // 수신 데이터를 FIFO 입력으로

wire fifo_ren;
assign fifo_ren = ren;        // 외부 ren 포트를 FIFO read enable로
...
fifo_16x8  rxd_fifo(
    ...
    .full  (fifo_full),
    .valid (fifo_valid),    // output wire full
