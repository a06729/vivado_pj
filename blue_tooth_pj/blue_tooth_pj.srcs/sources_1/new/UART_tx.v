...
module UART_tx(
    Clk, Rst_n, TxEn, TxData, TxDone, Tx, Tick, NBits
);
    input        Clk;
    input        Rst_n;
    input        TxEn;
    ...
    // Next state logic
    always @(State or D_edge or TxData or TxDone) begin
        case (State)
    ...
    always @(posedge Tick) begin
        ...
        if (write_enable) begin
            ...
            if ((counter == 4'b1111) & (!start_bit) & (Bit < NBits - 1)) begin
                in_data   <= {1'b0, in_data[7:1]};
                Bit       <= Bit + 1;
                Tx        <= in_data[0];
                start_bit <= 1'b0;
            ...
            if ((counter == 4'b1111) & (Bit == NBits - 1) & (!stop_bit)) begin
                Tx       <= 1'b1;
                counter  <= 4'b0000;
                stop_bit <= 1'b1;
            end
