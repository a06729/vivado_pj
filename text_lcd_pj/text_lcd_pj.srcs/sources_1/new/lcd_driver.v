...
module lcd_driver(
    ...
    reg [9:0] cnt_en_clk; 
     ...
     always @(posedge clk or negedge rst)begin
        if(!rst)begin
            ...
            if(cnt_init >= T_PW) cnt_init <= T_PW;
     ...
     always @(posedge clk or negedge rst)begin
        if(!rst) state<=IDLE;
        else if(en_clk) state<=next_state;
        else state<=state;
     end
     ...
     always @(*)begin
        case(state)
            IDLE:begin
                if(cnt_init>=T_PW) next_state<=FUNC_SET;
            ...
            PRINT_STRING:begin
                 if(index_char == 15) next_state <= LINE2;
                 else if (index_char == 31) next_state <= RETURN_HOME;
                 else next_state <= PRINT_STRING;
            end
    ...
    always @(posedge clk or negedge rst)begin
        if(!rst)  cnt_en_clk<=0;      
        else if(en_clk)  cnt_en_clk<=0;   
        else if(&cnt_en_clk) cnt_en_clk<=cnt_en_clk;
        else cnt_en_clk <= cnt_en_clk + 1'b1;
        
    end
    
     always@(posedge clk or negedge rst) begin
        if(!rst)  index_char <= 1'b0;
        else begin
         
         if(state==PRINT_STRING && en_clk == 1) begin 
            if(index_char<31) index_char<=index_char+1'b1;
            else index_char<=0; 
         end
         else index_char<=index_char;
     ...
     always@(posedge clk or negedge rst) begin
         if(!rst)  dly_en_clk<=0;
         else dly_en_clk<=en_clk;
     end
    
    always @(posedge clk or negedge rst) begin
        if(!rst)  lcd_e<=0;
        else if(state == IDLE) lcd_e<=0;
        else begin
             if(dly_en_clk)lcd_e<=1; 
             else if(&cnt_en_clk) lcd_e<=0;
             else  lcd_e<=lcd_e;
        end
    end
