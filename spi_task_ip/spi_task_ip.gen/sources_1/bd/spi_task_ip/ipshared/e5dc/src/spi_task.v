...
always @(posedge clock or negedge reset)begin
    ...
    else write_cnt <= ~s_write ? 4'b0 : (write_cnt==4'd15) ? 4'd15 : write_cnt+1'b1;
end
...
always @(posedge clock or negedge reset)begin
    ...
    else read_cnt <= ~s_read ? 4'b0 : (read_cnt==4'd15) ? 4'd15 : read_cnt+1'b1;
end
...
begin
    if(~reset) m_state <= 3'b0;
                    ...
                    (s_read   & (read_cnt==4'd15) & done_mst  ) ? M_IDLE : m_state ;
end
